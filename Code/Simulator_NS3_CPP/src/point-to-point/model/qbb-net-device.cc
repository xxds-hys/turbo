...
namespace ns3 {
		...
		QbbNetDevice::DequeueAndTransmit(void)
	{
		...
		Ptr<Packet> p;
		if (m_node->GetNodeType() == 0){  // for netdevice in host, has 8 virtual queues
			int qIndex = m_rdmaEQ->GetNextQindex(m_paused); // the index of the qp (NOT queue or virtual queue) to send next 
			if (qIndex != -1024){ // exist packet to send
				if (qIndex == -1){ // exist ack packets in the highest priority queue
					p = m_rdmaEQ->DequeueQindex(qIndex); // get the actual packet to send
